...
export const Bootstrap4 : (o?: PluginOptions) => RenderFormOptions = pluginOptions => ({
    layout: {
        formLayout(forObject: any, formConfig: WebForm, renderer: LayoutRenderer, validationResult?: FormValidationResult): AbstractDomElement {
            return BootstrapCommonLayout.formLayout(forObject, formConfig, renderer, this.renderFieldSet, validationResult)
        },
        renderFieldSet(field: FieldConfig, fieldValue: any, renderer: LayoutRenderer, validationResult?: ValidationResult): DomElementChildrenFrom {
            ...
            let validationMsg = validationResult && validationResult.hasError
                ...
                : null
            let fieldSet = vd('div', {class: `${field.type === 'checkbox' ? 'formConfig-check' : ''}`}, field.type === 'checkbox'
...
export const Bootstrap3 : (o?: PluginOptions) => RenderFormOptions = pluginOptions => ({
    layout: {
        formLayout(forObject, formConfig: WebForm, renderer: LayoutRenderer, validationResult?: FormValidationResult): AbstractDomElement {
            return BootstrapCommonLayout.formLayout(forObject, formConfig, renderer, this.renderFieldSet, validationResult)
        },
        renderFieldSet(field: FieldConfig, fieldValue: any, renderer: LayoutRenderer, validationResult?: ValidationResult): DomElementChildrenFrom {
            let _pluginOptions = {
            ...
            let validationMsg = validationResult && validationResult.hasError
                ...
                : null
            let fieldSet = vd('div', {class: `${field.type === 'checkbox' ? 'checkbox' : `formConfig-group ${validationResult && validationResult.hasError ? 'has-error' : ''}`}`}, field.type === 'checkbox'
                ? [...(input.constructor === Array ? input : [input]), label, ...[validationMsg]]
                : [label, ...(input.constructor === Array ? input : [input]), ...[validationMsg]])
            if (!isEmpty(field.helpText))
    ...
    layout: {
        formLayout(forObject, formConfig: WebForm, renderer: LayoutRenderer, validationResult?: FormValidationResult): AbstractDomElement {
            ...
            const inside = vd('table', {style: {borderCollapse: 'collapse', border: '1px solid lightgrey'}},
                vd('tbody', {},
                    Object.entries(formConfig.fieldsConfig).map((kv) => {
...
export const Bulma : (o?: PluginOptions) => RenderFormOptions = pluginOptions => ({
    layout: {
        formLayout(forObject, formConfig: WebForm, renderer: LayoutRenderer, validationResult?: FormValidationResult): AbstractDomElement {
            const result = renderer.formConfig(formConfig, {class: 'columns is-multiline'})
            validationResult ??= {hasError: false, message: '', fields: {}}
            for (const [fieldId, field] of Object.entries(formConfig.fieldsConfig)) {
        ...
        renderFieldSet(field: FieldConfig, fieldValue: any, renderer: LayoutRenderer, validationResult?: ValidationResult): DomElementChildrenFrom {
            ...
            return vd('div', {class: `${colClass} field ${field.type === 'checkbox' ? 'formConfig-check' : ''}`}, field.type === 'checkbox'
